...
while(cap.isOpened()):
    ...
    ret,thresh1 = cv2.threshold(resize_gray,100,255,cv2.THRESH_BINARY)
    ...
    if count %100 == 0: # clear terminal once awhile 
        os.system('cls')
    ...
    ratio = 1.2
    gray = cv2.resize(gray, (int(frame.shape[1]*ratio),int(frame.shape[0]*ratio )) ) 
